...
Phase 3B is live-read capable but not fully live-bound until final ENS text records are submitted and verified. `packages/ens-identity/src/live-resolver.ts` provides a live ENS resolver boundary using config-driven `ENS_PROVIDER_RPC` / `PRIVATE_EVM_RPC_URL`, `ENS_CHAIN_ID`, `ENS_NETWORK`, `ENS_NAME`, and optional `ENS_EXPECTED_POLICY_HASH`. `packages/center-cli/` exposes `identity live-status` in human-readable and deterministic JSON modes. `packages/ens-identity/src/bind-records.ts` now prepares one ENS Public Resolver `multicall(bytes[])` transaction for the ClearIntent records generated by `memory live-bindings`, so a future parent-wallet frontend can bind `agent.card`, `policy.uri`, `policy.hash`, `audit.latest`, and `clearintent.version` without separate record transactions or local ENS-owner key custody. Current operator evidence has reached live ENS name/address/version read and live 0G binding artifact generation; `ens-live-bound` remains pending until the multicall or manual ENS record update is submitted and `identity live-status` verifies the expected policy hash. Midpoint audit: `docs/audits/phase-3-ens-agent-identity/3b.5-midpoint-audit.md`.
...
Submit the prepared ENS text-record multicall or finish the equivalent manual ENS record update, set `CLEARINTENT_EXPECTED_POLICY_HASH`, and run `identity live-status` to capture `ens-live-bound` evidence. Then proceed to Phase 4B live KeeperHub/onchain execution evidence. Phase 5C signer validation should be repeated as `software-wallet-tested testnet-integrated` only after 3B and 4B live-testnet evidence exists.
...
- Phase 3B now has a live read route and awaits operator ENS RPC/name/record configuration before it can claim `ens-live-read` or `ens-live-bound`.
...
- Phase 2B boundary: smoke storage proof only; final demo policy/audit artifacts still need upload and ENS binding during Phase 3B/demo work
...
2. Implement Phase 3B live/testnet ENS binding against the proven 2B storage capability.
3. Implement Phase 4B live/testnet KeeperHub/onchain execution against the proven adapter semantics.
4. Repeat Phase 5C as testnet-integrated after 3B/4B live evidence, then run Phase 5D/5E WalletConnect and hardware utilization validation when the operator devices/sessions are available.
...
- one agent can prepare Phase 3B live ENS binding using the recorded Phase 2B smoke evidence
