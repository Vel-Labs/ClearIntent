...
export function getAlchemyReadiness(env: Record<string, string | undefined> = readProcessEnv()): AlchemyReadiness {
  const config: AlchemyPublicConfig = {
    ...
    policyIdOrClientId: readFirst(env, "NEXT_PUBLIC_ALCHEMY_POLICY_ID_OR_CLIENT_ID", "NEXT_PUBLIC_ALCHEMY_CLIENT_ID"),
  ...
  return {
    ...
    config,
    missing: [],
    notes: ["Alchemy Account Kit public configuration is present; this is readiness only, not authority proof."]
  };
}

function requiredAlchemyFields(config: AlchemyPublicConfig): Array<keyof AlchemyPublicConfig> {
  return (["chain", "policyIdOrClientId"] as const).filter((key) => !config[key]);
}
